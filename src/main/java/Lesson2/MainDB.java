...
public class MainDB {
    ...
    public static void main(String[] args) {
        try {
            ...
            insertDB("Bob10", 100);
            System.out.println(getScoreDB("Bob10"));
            deleteDB("Bob10");

            createDB("CREATE TABLE students3 (\n" +
                    "    id    INTEGER PRIMARY KEY AUTOINCREMENT,\n" +
                    "    name  TEXT,\n" +
                    "    score INTEGER\n" +
                    ")");

            trancateDB("students2");
...
//            }
    ...
    //------------DZ start--------------
    public static void createDB(String DDL) {
        String sql = String.format(DDL);
        try {
            stmt.executeUpdate(sql);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public static void trancateDB(String tableName) {
    ...
    public static void insertDB(String name, int score) {
    ...
    public static int getScoreDB(String name) {
        String sql = String.format("SELECT score FROM students WHERE name = '%s'", name);
        try {
